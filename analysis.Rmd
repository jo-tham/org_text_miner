...
org_terms <- names(org_term_freq[tail(org_term_order, n=25L)])
org_terms
agenda_corpus_xorg <- tm_map(agenda_corpus, removeWords,
                             org_terms)
proj_doc_term_matrix <- DocumentTermMatrix(agenda_corpus_xorg)
proj_term_freq <- colSums(as.matrix(proj_doc_term_matrix))
```
...
```{r}
findFreqTerms(proj_doc_term_matrix, lowfreq=1000)
findFreqTerms(proj_doc_term_matrix, lowfreq=100)
findFreqTerms(proj_doc_term_matrix, lowfreq=10)
```
...
set.seed(1945)
wordcloud(names(proj_term_freq), proj_term_freq,
          min.freq=40,
