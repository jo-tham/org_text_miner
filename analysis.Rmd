...
- There's a hack in here to plot only some of the words,
  ...
  work. I heard there's a bug with that if they're at the
...
# Again, for each agenda file

```{r}
setwd("data")
org_files <- dir()
corp_to_wordcloud <- function(source_file){
    my_source <- readPlain(list("content"=source_file),
                           language="en", id="id")
    agenda_corpus <- VCorpus(x=my_source,
                             readerControl=
                                 list(reader = reader(my_source),
                                      language = "en"))
    agenda_corpus <- tm_map(agenda_corpus,
                            content_transformer(removeWords),
                            stopwords())
...
lapply(org_files, corp_to_wordcloud)
```
